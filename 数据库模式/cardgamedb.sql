...
CREATE TABLE card_pool_types (
    ...
    pool_type_name VARCHAR(50) NOT NULL UNIQUE   -- 卡池类型名称（如普通池、fes池）
);
...
CREATE TABLE card_pools (
    ...
    rarity_probabilities JSON NOT NULL,         -- 稀有度出货概率（JSON存储，例如 {"R": 0.8, "SR": 0.15, "SSR": 0.05}）
    guarantee_count INT NOT NULL DEFAULT 0,     -- 保底次数（如200抽必出SSR）
    FOREIGN KEY (pool_type_id) REFERENCES card_pool_types(pool_type_id) ON DELETE CASCADE
);

-- 用户卡池统计表：记录用户在每个卡池中的累计抽卡次数
CREATE TABLE user_pool_stats (
    stat_id INT AUTO_INCREMENT PRIMARY KEY, -- 统计记录唯一标识
    user_id INT NOT NULL,                   -- 用户ID
    pool_id INT NOT NULL,                   -- 卡池ID
    draw_count INT DEFAULT 0,               -- 累计抽卡次数
    since_last_guarantee INT DEFAULT 0,     -- 自上次触发保底后的抽数
    last_draw_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP, -- 上次抽卡时间
    FOREIGN KEY (user_id) REFERENCES users(user_id) ON DELETE CASCADE,
...
-- 卡牌分解记录表：记录用户分解卡牌的历史
CREATE TABLE card_dismantle_history (
    dismantle_id INT AUTO_INCREMENT PRIMARY KEY, -- 分解记录唯一标识
    user_id INT NOT NULL,                        -- 用户ID
    card_id INT NOT NULL,                        -- 分解的卡牌ID
    resource_gained JSON,                        -- 获得的资源（JSON存储）
    dismantle_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP, -- 分解时间
    FOREIGN KEY (user_id) REFERENCES users(user_id) ON DELETE CASCADE,
    FOREIGN KEY (card_id) REFERENCES cards(card_id) ON DELETE CASCADE
);
...
CREATE TABLE combo_skills (
    combo_skill_id INT AUTO_INCREMENT PRIMARY KEY, -- 组合技能唯一标识
    team_id INT NOT NULL,                          -- 队伍ID
    skill_name VARCHAR(100) NOT NULL,              -- 技能名称
    FOREIGN KEY (team_id) REFERENCES battle_teams(team_id) ON DELETE CASCADE
);
...
CREATE TABLE combo_conditions (
    ...
    combo_skill_id INT NOT NULL,                 -- 组合技能ID
    required_card_type VARCHAR(50) NOT NULL,     -- 所需卡牌类型
    FOREIGN KEY (combo_skill_id) REFERENCES combo_skills(combo_skill_id) ON DELETE CASCADE
);
